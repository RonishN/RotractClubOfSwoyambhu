...
export default function PriorityEventModal({ event }) {
  ...
  useEffect(() => {
    ...
    const dismissKey = `dismissed_priority_event_${event.id}`;
  ...
  const handleDontShowAgain = () => {
    localStorage.setItem(`dismissed_priority_event_${event.id}`, '1');
    handleClose();
  };
  ...
  return (
    <>
      <style>{`
        @keyframes pem-backdrop-fade { from { opacity: 0 } to { opacity: 1 } }
        @keyframes pem-card-pop { 
          0% { opacity: 0; transform: scale(0.92) translateY(20px); } 
          100% { opacity: 1; transform: scale(1) translateY(0); } 
        }
        @keyframes pem-card-pop-out { 
      ...
      <div
        ...
        style={{
          ...
          animation: animatingOut ? 'pem-backdrop-fade 0.3s ease reverse' : 'pem-backdrop-fade 0.3s ease forwards',
      ...
      >
        ...
        <div
          ...
          style={{
            ...
            animation: animatingOut ? 'pem-card-pop-out 0.3s cubic-bezier(0.4,0,0.2,1) forwards' : 'pem-card-pop 0.4s cubic-bezier(0.16, 1, 0.3, 1) forwards',
            position: 'relative',
            display: 'flex',
        ...
        >
          ...
          {/* Featured Badge */}
          <div className="pem-badge-glow" style={{
            position: 'absolute', top: 14, left: 14, zIndex: 20,
            background: 'linear-gradient(135deg, #f59e0b, #d97706)',
            color: '#ffffff', fontSize: '0.72rem', fontWeight: 800,
            padding: '5px 13px', borderRadius: 20, letterSpacing: '0.5px',
          ...
          {/* Cover image preserving original aspect ratio (16:9 box container with clean cover/contain fallback) */}
          {coverImg && (
